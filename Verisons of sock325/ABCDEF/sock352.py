...
def ResendPackets(delay,self):
   ...
   # print('im here')
    ...
    while (True):
        #print('im here')
        if(len(list_of_outstanding_acks) == 0):
            return
        # example

        if len(list_of_outstanding_acks) != 0:
             for i in range(len(list_of_outstanding_acks)):
                #print('reached')
                newPacket = list_of_outstanding_acks[i]
                newPacket.cntl = ACK
                ...
                list_of_outstanding_acks.remove(list_of_outstanding_acks[i])
                break
        ...
        for packet in list_of_outstanding_packets:
            ...
            if (time_diff > delay):
               ...
               # print('Packet is being retransmitted')
...
# receive a message up to MAX_DATA
    ...
    def recvfrom(self,nbytes):
      ...
      # print(packet.seq)
       if packet.seq == expectedseq+1:
           ...
           #print('raeched2')
           list_of_outstanding_acks.append(packet)
    ...
    def sendclosingpacket(self):
        ...
        if (self.serveraddress == 0):
            ...
            self.mysocket.sendto(newPackedPacket, self.serveraddress)
        AA = skbuf(packet, time.clock())
        #list_of_outstanding_packets.append(AA)
        #print("\n")
        #print(len(list_of_outstanding_packets))
        #print(list_of_outstanding_packets[0].Packet.seq)
        #print(len(list_of_outstanding_acks))
